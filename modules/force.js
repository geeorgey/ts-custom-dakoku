...
let sfrequest = (oauth, path, options) => new Promise((resolve, reject) => {
    ...
    logger.info('[info]', { 'before': 'reqest options' + JSON.stringify(options) });
...
let apexrest = (oauth, path, params) => {
    logger.info('[info]', { '■apexrest oauth■': oauth });
    logger.info('[info]', { '■apexrest params': params });
    logger.info('[info]', { '■apexrest params.body': JSON.stringify(params.body) });
    //oauthに入るデータは
    //access_token
    //refresh_token
    //signature
    ...
    logger.info('[info]', { '■apexrest path■': path });
